...
/****************************************************************************/
/****************************************************************************/
LAURealSenseGLFilter::LAURealSenseGLFilter(unsigned int cols, unsigned int rows, QObject *parent) : LAUAbstractGLFilter(cols, rows, parent), faceTexture(nullptr), depthTexture(nullptr), frameBufferObjectA(nullptr), frameBufferObjectB(nullptr), frameBufferObjectC(nullptr), frameBufferObjectD(nullptr), textureLookUpTable(nullptr)
{
    ;
}
...
/****************************************************************************/
/****************************************************************************/
LAURealSenseGLFilter::~LAURealSenseGLFilter()
{
    ...
    qDebug() << QString("LAURealSenseGLFilter::~LAURealSenseGLFilter()");
}

/****************************************************************************/
/****************************************************************************/
/****************************************************************************/
void LAURealSenseGLFilter::initialize()
{
    ...
    videoTexture->setSize(static_cast<int>(numCols), static_cast<int>(numRows));
    ...
    if (portrait()) {
        ...
        frameBufferObjectC = new QOpenGLFramebufferObject(static_cast<int>(numRows), static_cast<int>(numCols), frameBufferObjectFormat);
    ...
    if (lookUpTable.isValid()) {
        ...
        // UPLOAD THE TEMPORARY MEMORY OBJECT TO THE GPU
        textureLookUpTable = new QOpenGLTexture(QOpenGLTexture::Target3D);
...
/****************************************************************************/
/****************************************************************************/
void LAURealSenseGLFilter::setLookUpTable(LAULookUpTable lut)
{
...
/****************************************************************************/
/****************************************************************************/
void LAURealSenseGLFilter::setFace(const unsigned char *buffer)
{
    if (surface && makeCurrent(surface)) {
        ...
        faceTexture->setData(QOpenGLTexture::Red, QOpenGLTexture::UInt8, reinterpret_cast<const void *>(buffer), &options);
...
/****************************************************************************/
/****************************************************************************/
void LAURealSenseGLFilter::setDepth(const unsigned short *buffer)
{
    if (surface && makeCurrent(surface)) {
        // BIND THE VERTEX ARRAY OBJECT
        vertexArrayObject.bind();
                ...
                if (programB.bind()) {
                    if (quadVertexBuffer.bind()) {
                        if (quadIndexBuffer.bind()) {
                            ...
                            glActiveTexture(GL_TEXTURE0);
                            faceTexture->bind();
                            programB.setUniformValue("qt_nirTexture", 0);

                            // SET THE DEPTH TEXTURE ON THE GPU
                            glActiveTexture(GL_TEXTURE1);
                            depthTexture->bind();
                            programB.setUniformValue("qt_dptTexture", 1);
...
/****************************************************************************/
/****************************************************************************/
void LAURealSenseGLFilter::setVideo(const unsigned char *buffer)
{
    if (surface && makeCurrent(surface)) {
        // BIND THE VERTEX ARRAY OBJECT
        vertexArrayObject.bind();
        ...
        // UPLOAD THE FRAME TO THE GPU
        videoTexture->setData(QOpenGLTexture::RGB, QOpenGLTexture::UInt8, reinterpret_cast<const void *>(buffer), &options);
...
/****************************************************************************/
/****************************************************************************/
void LAURealSenseGLFilter::scanObject(float *buffer)
{
    if (surface && makeCurrent(surface)) {
        if (frameBufferObjectB) {
            glBindTexture(GL_TEXTURE_2D, frameBufferObjectB->texture());
