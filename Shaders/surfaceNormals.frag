...
void main()
{
    ...
    if (coord.x%2 == 0){
        // PASS THROUGH THE ORIGINAL SCAN TO THE FBO
        qt_fragColor = texelFetch(qt_texture, coord/ivec2(2,1), 0);
    } else {
        // POINT THE FRAGMENT COORDINATE TO THE SCAN TEXTURE
        coord.x = coord.x/2;

        // GET THE SUBJECT PIXEL FROM THE SCAN TEXTURE
    ...
    qt_fragColor.w = float(isnan(qt_fragColor.z) == false);
